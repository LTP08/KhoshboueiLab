...
FRAME_WIDTH = 0
save_video_path = ""
record_enabled = None  # Variable to track recording status, default to False


def init_camera():
    global system, cam, video_label, video_writer, FRAME_HEIGHT, FRAME_WIDTH, save_video_path, record_enabled
    try:
        ...
        save_video_path = save_directory if save_directory else os.path.expanduser("~")
...
def choose_directory():
    global save_directory, video_writer
    new_directory = filedialog.askdirectory()
    if new_directory:
        save_directory = new_directory
        directory_label.config(text=f"Save Directory: {save_directory}")
        if video_writer is not None:
            # Update the path of the video writer
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            video_filename = os.path.join(save_directory, f"video_{timestamp}.mp4")


def camera_acquisition():
    global idle_status, cam, record_enabled, video_writer
...
def start_recording_thread():
    global acquisition_thread, idle_status, cam, record_enabled
...
def stop_recording():
    global acquisition_thread, idle_status, video_writer
    idle_status = True
    if video_writer is not None:
...
record_enabled = BooleanVar(value=False)
record_checkbox = Checkbutton(root, text="Record", variable=record_enabled)
record_checkbox.pack()
