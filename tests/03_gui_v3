...
def camera_acquisition():
    ...
    while not idle_status:
        print("Acquisition loop running...")
        if cam is None:
            break  # Safety check
        ...
        root.update()
    if cam is not None:
        if cam.IsStreaming():  # Check if the camera is streaming
            cam.EndAcquisition()  # End the camera acquisition
    print("Acquisition loop exited.")
...
def stop_recording():
    global acquisition_thread, idle_status
    print("Entering stop_recording(). Idle status:", idle_status)
    idle_status = True
    print("here")
    print("Stop recording called. Idle status:", idle_status)
